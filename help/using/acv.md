...
`ACV` (Convalida del contenuto delle risorse) Identifica i nodi obbligatori mancanti e le violazioni nel contenuto delle risorse. Alcune funzionalità di Assets su Experience Manager as a Cloud Service potrebbero non funzionare.
...
* `psb.file.large`: identifica file PSB di grandi dimensioni (`dc:format : application/vnd.3gpp.pic-bw-small`) di dimensioni superiori a 2 GB.
* `invalid.asset.name`: identifica le risorse con caratteri non validi[`* / : [ \ ] | # % { } ? &`] nel nome.

## Possibili implicazioni e rischi {#implications-and-risks}

* Può causare errori in alcune funzioni di Assets che dipendono dalle proprietà ereditate in Experience Manager as a Cloud Service.
...
* Un numero elevato di discendenti sotto un nodo di metadati potrebbe rallentare il download delle cartelle che contengono risorse che violano.
* La presenza di nodi in conflitto potrebbe causare un errore di acquisizione su AEM as a Cloud Service.
...
* Non è richiesta alcuna azione in caso di rappresentazione originale di risorse secondarie mancanti.
